/**
   @file RAK4631_Get_Addr.ino
   @author Bernd Giesecke (bernd.giesecke@rakwireless.com)
           for the original ble_uart.ino
           Additional code (getAddr) by Kongduino
   @brief BLE example shows how to implement UART over BLE
          Sets the device name's to "nRF52 BLE xx xx xx xx xx xx"
   @version 0.1
   @date 2020-07-27, update 2022-03-18
...
void setup() {
  ...
  uint8_t mac[6];
  uint8_t rslt = Bluefruit.getAddr(mac);
  char prnt[128];
  sprintf(prnt, "Your BLE MAC ID is: %2x %2x %2x %2x %2x %2x\n\n", mac[5], mac[4], mac[3], mac[2], mac[1], mac[0]);
  Serial.println(prnt);
  sprintf(prnt, "nRF52 BLE %2x %2x %2x %2x %2x %2x", mac[5], mac[4], mac[3], mac[2], mac[1], mac[0]);
  Bluefruit.setName(prnt);
